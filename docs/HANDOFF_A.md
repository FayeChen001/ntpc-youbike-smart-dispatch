...
（檔名沿用 `HANDOFF_A.md` 以免打斷其他兩條線的引用；內文一律用中文名稱：政府端／微笑單車／用戶端。）
...
| I02（政府端可見部分） | 同站兩台不同車號建單 | R001／R002 未合併；同車號重送合併為同一張 **通過** |
| A-5 | 將 R001 推進到 `recovered` | 政府端顯示差異「已處理但服務未恢復」；另偵測到 R003「沒有資產識別」 **通過** |
...
從 政府端的視角跑完整閉環，不繞過任何一端的實作。測試對伺服器既有狀態無依賴
（每次用唯一車號），連跑兩次都 ALL PASS。

| 步驟 | 驗到什麼 | 結果 |
|---|---|---|
| 1 | 用戶端送出機械症狀回報 → 直接開出工單，回傳 `report_id` 與 `ticket_id` | **通過** |
| 1b | 同 `request_id` 重送 → 冪等，仍是同一個 `ticket_id`，不重複建單 | **通過** |
| 2 | 政府端看到同一個 `ticket_id`、車號、`dock_id`；三欄分開（①有②無③無）；設備＝疑似故障、服務＝未知；`report_ids` 有回連 | **通過** |
| 3 | 微笑單車推進 `accepted`→`on_site`：政府端流程狀態同步、看到營運負責人、③現場確認出現 | **通過** |
| 4 | 微笑單車推進 `recovered`：設備＝已維修但**不是驗收正常**、服務仍未知、政府端標出「已處理但服務未恢復」 | **通過** |
| 5 | 微笑單車推進 `verified` 才是 `verified_ok` | **通過** |
| 6 | 全程同一個 `ticket_id` 只出現一次，去重依據是車號 | **通過** |
| 7 | 同站不同車另開一張單，政府端兩張並存（I02） | **通過** |
...
依分工規則這屬於共用邏輯，我沒有自行修改，請微笑單車或當輪整合者處理。
...
| ~~AI 圖片觀察實際顯示~~ | **已驗證**（`78fd521`） | 以 用戶端的 `image_analysis` 介面送入測試樁資料驗過欄位切分。**尚未用真實 Bedrock 視覺模型跑過** |
| **真實視覺模型** | **未驗證** | 上述測試用的是測試樁；C 的 `report_image` 接真模型後需再驗一次 |
| **微笑單車端是否顯示政府端要求** | **未驗證** | A 已送出帶 `event_id`／`event_version` 的 ops 通知並確認進入 `/api/notifications?channel=ops`；B 畫面是否呈現、是否回填處理方案，需微笑單車確認 |
...
| **`app/server.py` 的 availability 接線** | **未提交** | （已解決）當時該檔有用戶端未提交的改動不能 `git add`，後續已串行提交完成 |
| **獨立把關兩輪** | **未執行** | 本文件是 政府端自測加一條真實三端閉環，仍不等於獨立把關 |
| **微笑單車端畫面是否呈現政府端決策** | **未驗證** | A 的決策留在事件上並送 ops 通知，B 是否顯示需微笑單車確認 |
...
政府端本輪要做的都做完了，對目前 main 重驗過：五套測試全過（離線 fixture 四套不需伺服器，
...
app/server.py          只有 `# 政府端（政府端）` 那一段與 api_ledger 內的兩行接線
```
...
1. **主管決策卡的方案比較不完整** — 卡在微笑單車沒有改派方案端點（`/api/ops/contract` 仍是 `b-1`，
   只涵蓋工單）。需要微笑單車提供：就近改道與跨區支援各自的抵達時間、原任務延後幾分鐘、增加里程、
   車源來自哪一區、受影響站點。**政府端不會自行估算，沒有這支端點卡片只能顯示既有任務。**
2. **task 缺 `version`／`event_id`／`operator_owner`** — `overview.tasks.contract_missing` 會如實列出。
3. **真實視覺模型未驗** — AI 欄位是用 用戶端的 `image_analysis` 介面送測試樁資料驗的，
   用戶端接上 Bedrock 後要再驗一次。
4. **微笑單車端是否呈現政府端的決策與要求未驗** — A 這邊送出並留痕了，也確認進入 ops channel，
   B 畫面需微笑單車確認。
5. **獨立把關兩輪未執行** — 本檔是 政府端自測加一條真實三端閉環，不等於獨立把關。

## 5. 仍未修的共用程式缺陷（已回報三次，不是 政府端的檔案）
